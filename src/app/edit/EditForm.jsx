  ...
  useEffect(() => {
    if (index !== null) {
      ...
      if (!isNaN(idx) && data[idx]) {
        ...
        router.push("/"); // กลับหน้าหลักถ้า index ผิด
      }
    }
  }, [index]);
  ...
  return (
    <div className="min-h-screen">
      <div className="flex items-center justify-center gap-5 p-5">
        ...
        <h1 className="text-center text-[40px] md:text-[50px] font-bold text-[#FE5E25]">
      ...
      <div className="flex justify-center px-5 md:px-20">
        <form className="grid grid-cols-1 md:grid-cols-2 md:gap-5" onSubmit={handleSubmit}>
          <div className="flex flex-col gap-3">
            {["carNumber", "driverName", "startStation", "endStation", "contact"].map((field) => (
              <div key={field}>
                ...
                <input
                  type="text"
                  className="w-full md:w-[300px] h-[40px] bg-white border border-gray-400 rounded-md p-2"
                  placeholder={
                    field === "carNumber"
                      ? "B101-01"
                      : field === "driverName"
          ...
          <div className="md:col-span-2 flex justify-end gap-5 mt-5">
            <Link href="/">
              <button type="button" className="w-[130px] bg-[#C5C5C5] text-white px-6 py-3 rounded-4xl hover:bg-gray-500 transition">
                ยกเลิก
              </button>
            </Link>
            <button
